...
| 2 | Assigning IP addresses to `R1`'s `Gi0/0/0` and `Se0/1/0` via `ip address <IP address> <IP subnet mask>` reveals both interfaces are administratively down — `show ip interface brief` confirms this | Issued `no shutdown` on both interfaces; repeated procedure for `R2` and `R3` | All interfaces assigned IP addresses within their respective subnets; all links active and OSPF adjacencies formed between routers according to SYSLOG messages | <img src="Elements/step2.png"> |
