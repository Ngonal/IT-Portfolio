...
## Bonus Tips
### Tip #1 - The `show ip interface brief` command provides a quick health check of all interfaces. A state of **`down/down`** indicates a Layer 1 issue; however, it does not definitively mean that no cable is connected. It is important to visually verify that the appropriate cable — data, console, or power — is physically connected and properly seated in the relevant ports:
