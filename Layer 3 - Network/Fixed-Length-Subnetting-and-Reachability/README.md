...
| 2 | Proceeding to assign first and last usable IP addresses to the designated interfaces of every Router using the `ip address <IP address> <UP subnet mask` starting with `R1`'s `Gi0/0/0` and `Se0/1/0`, we can see that the links remain down | After viewing the status of the respective interfaces, we can see that the router ports of R1 are administratively disabled — we enable them and the links are now up; we proceed to perform the same procedure for Routers R3 and R2 | All interfaces now assigned appropriate Ip addresses within their respective subnets; all links active and all routers have formed OSPF adjancencies with eachother | <img src="Elements/step2.png"> |
